			...
			<form method="get">
				...
				<select name='memo'>
					...
					<?php
						...
						$Query="SELECT * FROM memos WHERE Status=0";
				...
				<table class="Memo">
					<tr>
						...
						<select ng-model="MemoInfo[0].Group_Style" ng-change='Select_GroupStyle()' required>
							<option ng-repeat='y in Assignment | filter:{Vendor_id:MemoInfo[0].Vendor}'>{{y.GroupStyle_Name}}</option>
						</select>
						</td>
					</tr>
					<tr>
		...
		.controller('Table_Ctrl', function($scope,$window, $http,$timeout) {
			...
			$scope.AddRows=function(){ 
				for(var i=0;i<$scope.Lines;i++) {
					if ($scope.range.length<150){
							$scope.range.push({Memo:$scope.MemoInfo[0].Memo,SS_Quality:$sope.MemoInfo[0].SS_Quality,Item:$scope.range.length+1,SSS_Type:"",Sku:"",Cut:"",Color:"",Clarity:"",Quantity:1,Weight:0,Cost:0});
					}else{
			...
			$scope.Import=function(){
				...
				for (i=0; i<clipRows.length - 1; i++) {
					   $scope.range.push({Memo:$scope.MemoInfo[0].Memo,SS_Quality:$scope.MemoInfo[0].SS_Quality,SSS_Type:$scope.MemoInfo[0].SSS_Type,Item:$scope.range.length+1,Sku:clipRows[i][0],Cut:clipRows[i][1],Color:clipRows[i][2],Clarity:clipRows[i][3],Qty:parseInt(clipRows[i][4]),Weight:parseFloat(clipRows[i][5]),Cost:parseFloat(clipRows[i][6])})
			...
			$scope.file_changed = function(element) {
				 ...
				 $scope.$apply(function(scope) {
					...
					reader.onload= function(e) {
						$scope.PictureFile[0].Name=$scope.MemoInfo[0].GroupStyle;
